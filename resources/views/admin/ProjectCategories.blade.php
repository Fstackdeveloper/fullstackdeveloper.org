...
$(function() {
    $('#projects-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route('project.category.data') !!}',
        columns: [
            { data: 'id', name: 'id' },
			...
			{"mRender": function ( data, type, row, meta ) {
                        return '<a href="/admin/project/category/edit/'+row.id+'" class="btn btn-circle green">Edit <i class="fa fa-edit"></i></a>'+
							   '<a href="{{ url('/admin/project/category/destroy') }}/'+row.id+'" onclick="event.preventDefault();document.getElementById(\'destroy-form'+row.id+'\').submit();" class="btn btn-circle red" >Delete <i class="fa fa-trash-o"></i></a><form id="destroy-form'+row.id+'" action="{{ url('/admin/project/category/destroy') }}/'+row.id+'" method="POST" style="display: none;"><input type="hidden" name="_token" value="{{ csrf_token() }}"></form>';
			}}
        ]
    });
});
