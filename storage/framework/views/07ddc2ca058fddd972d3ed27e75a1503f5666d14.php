...
$(function() {
    $('#projects-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo route('portfolio.data'); ?>',
        columns: [
            { data: 'id', name: 'id' },
			...
			{"mRender": function ( data, type, row, meta ) {
                        return '<a href="/admin/portfolio/edit/'+row.id+'" class="btn btn-circle green">Edit <i class="fa fa-edit"></i></a>'+
							   '<a href="<?php echo e(url('/admin/portfolio/destroy')); ?>/'+row.id+'" onclick="event.preventDefault();document.getElementById(\'destroy-form'+row.id+'\').submit();" class="btn btn-circle red" >Delete <i class="fa fa-trash-o"></i></a><form id="destroy-form'+row.id+'" action="<?php echo e(url('/admin/portfolio/destroy')); ?>/'+row.id+'" method="POST" style="display: none;"><input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>"></form>';
			}}
        ]
    });
});
