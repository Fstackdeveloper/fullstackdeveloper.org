...
$(function() {
    $('#projects-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo route('message.data'); ?>',
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
			...
			{"mRender": function ( data, type, row, meta ) {
                        return '<a href="/admin/message/'+row.id+'" class="btn btn-circle green">Show <i class="fa fa-eye"></i></a>'+
							   '<a href="<?php echo e(url('/admin/message/destroy')); ?>/'+row.id+'" onclick="event.preventDefault();document.getElementById(\'destroy-form'+row.id+'\').submit();" class="btn btn-circle red" >Delete <i class="fa fa-trash-o"></i></a><form id="destroy-form'+row.id+'" action="<?php echo e(url('/admin/message/destroy')); ?>/'+row.id+'" method="POST" style="display: none;"><input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>"></form>';
			}}
        ]
    });
});
